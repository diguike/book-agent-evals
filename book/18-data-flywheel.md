...
- **招牌菜 2 数据飞轮**：线上日志 → 自动挖 hard case → PII scrub → 人工筛 → 加入评测集 → 改 prompt → 重跑评测 → 闭环
...
- **招牌菜 1A**（第 4 章）：从 0 造 60 条 L1 种子集
- **招牌菜 1B**（第 17 章）：扩到 200 条 + 版本管理
- **招牌菜 2**（这一章）：数据飞轮，让评测集持续从线上自动扩充
